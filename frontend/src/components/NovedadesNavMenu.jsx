...
import { NOVEDADES_ITEMS, isNovedadesPath, itemsForRole } from "../config/navigation";
...
export function NovedadesNavMenu() {
  const { user } = useAuth();
  ...
  const visibleItems = itemsForRole(NOVEDADES_ITEMS, user?.role);
  const parentActive = isNovedadesPath(location.pathname);
