...
La app MUST ofrecer menú **Agenda ocupación** en `/agenda-ocupacion` para `admin`/`operador`.

La pantalla MUST:

- Cargar events al cambiar la ventana del calendario (`datesSet`).
- Mostrar filtros dominio, tipo, especialidad, médico, día.
- Usar FullCalendar (día/semana/mes); un solo color de evento; título = médico.
- Al click, mostrar popover/tooltip con el detalle extendido.
- Ser solo lectura (sin crear/editar/borrar sync ni bookings).
- NO invocar sync; el sync permanece en Ocupación.

`/agenda` MUST permanecer sin cambios funcionales por este change.

#### Scenario: Navegación de semana

- **Given** usuario en Agenda ocupación
- **When** cambia a la semana siguiente
- **Then** se vuelve a pedir events con el nuevo `start`/`end`
