...
El backend MUST persistir el snapshot completo del endpoint externo en `ocupacion_horario_activo` con **PK serial local** (rev `0013`; `id_dato` del API **no** es único — una fila DB por cada fila JSON), `payload` JSONB, y columnas derivadas `tipo`/`especialidad_agenda`/`medico`. `POST .../horarios-activos/sync` MUST, tras GET externo OK, wipe+reload en una sola transacción. Si el GET falla, MUST NOT modificar la tabla. `GET .../horarios-activos` MUST leer de DB y filtrar `fecha_hasta >= hoy`. Ambos endpoints MUST exigir JWT + `admin`/`operador`. El token MUST NOT devolverse al cliente.

*(Cierre 2026-08-06: ver `ARCHIVE.md` y `openspec/specs/distribucion/spec.md`.)*

#### Scenario: Sync OK

- **Given** URL/token configurados y la API externa responde OK
- **When** un operador hace POST sync
- **Then** la tabla queda con exactamente las filas del payload (1:1, sin colapsar por `id_dato`) y la respuesta incluye `synced`
...
El backend MUST derivar `tipo`, `especialidad_agenda` y `medico` partiendo `nombre_agenda` por el separador ` - `. Si no hay ese separador y el valor contiene `-`, MUST usar fallback partiendo por `-` (partes strip). Parte 1 → `tipo`, parte 2 → `especialidad_agenda`, resto unido → `medico`. Partes faltantes MUST ser null. La grilla MUST NOT mostrar `nombre_agenda` crudo.

*(Ampliación 2026-08: valores compactos del API; ver `openspec/changes/locations-tipo` y `agenda-ocupacion-ui/implementation-notes.md`.)*
