...
Servicios MUST include optional integer **concepto_liquidacion** (label "Concepto liquidación"). Empty or `0` MUST persist as `NULL`. A non-zero value MUST be an integer ≥ 1 (no extra upper bound). Negative values MUST be rejected (422). Duplicate values across servicios MUST be allowed.

Admin/`rrhh` MUST manage servicios in Parametrización like Módulos: grid, **Nuevo servicio** modal (Cancelar/Cargar; always `activo=true`), edit modal (nombre, valor hora, concepto, **Activo** checkbox), confirm-delete modal; Escape cancels. Inline `valor_hora` edit MUST be removed. The grid MUST show `#id · nombre · activo · Concepto liquidación` (`NULL` → "—") and valor hora.
...
#### Scenario: Concepto negativo rechazado

- GIVEN payload con concepto_liquidacion = -1
- WHEN POST o PUT `/servicios`
- THEN MUST rechazar (422)

#### Scenario: Alta servicio siempre activo

- GIVEN `rrhh` en Nuevo servicio
- WHEN carga los datos
- THEN el servicio MUST crearse con `activo=true` (sin checkbox Activo en el modal de alta)

#### Scenario: Editar servicio con Activo

- GIVEN servicio existente
- WHEN abre modal editar
- THEN MUST ver checkbox Activo (pre-seleccionado según estado actual)
- AND puede desmarcar para desactivar

#### Scenario: Confirmar eliminación de servicio

- GIVEN servicio en la grilla
- WHEN pulsa eliminar y confirma en el modal
- THEN MUST soft-delete el servicio
...
#### Scenario: Cambio de fecha invalida módulo

- GIVEN módulo Semana seleccionado, fecha = martes
- WHEN cambia fecha a sábado
- THEN MUST limpiar el módulo seleccionado (ya no es válido)
...
#### Scenario: Eliminar feriado con confirmación

- GIVEN feriado cargado
- WHEN pulsa eliminar y confirma en el modal
- THEN MUST soft-delete
- AND Cancelar o Escape MUST NOT eliminar
