...
Roles de usuario del panel clínico y reglas de autorización asociadas (JWT + ABM usuarios, correo y recuperación de contraseña).

## Requirements

### Requirement: User role enum

User role validation MUST accept four values: `admin`, `operador`, `jefe_medico`, `rrhh`. Create/update MUST aceptar esos roles. Update MAY incluir email y contraseña opcional (bcrypt). JWT/`/me` MUST expose the role. Password rules and generic auth failures MUST remain unchanged. Fallos de auth MUST seguir siendo genéricos.
...
La pantalla Usuarios MUST mostrar botón **Nuevo usuario** y una grilla de usuarios existentes con el formato de grillas de la app. Acciones de modificar MUST estar a la derecha. Solo rol `admin` MUST acceder.
...
- THEN MUST ser denegado (UI/ruta)
